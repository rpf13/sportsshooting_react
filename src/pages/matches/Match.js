      ...
      setMatches((prevMatches) => ({
        ...
        results: prevMatches.results.map((match) => {
          return match.id === id
            ? {
                ...match,
                attendings_count: match.attendings_count + 1,
        ...
        <Card.Body>
          ...
          {title && (
            <Card.Title className="text-center">
              {title} - <FormatDay match_date={match_date} />
            </Card.Title>
          )}
          {title && (
            <Card.Text className="text-center">{match_location}</Card.Text>
          )}
          {title && (
            <Card.Text className="text-center">{level_filter}</Card.Text>
          )}
          ...
          <div className={styles.MatchBar}>
            ...
            {attending_id ? (
              <span onClick={handleUnattend}>
                <i
                  className={`fa-regular fa-calendar-check ${styles.Attend}`}
                />
              </span>
            ) : currentUser ? (
              <span onClick={handleAttend}>
                <i
                  className={`fa-regular fa-calendar-check ${styles.AttendOutline}`}
                />
              </span>
            ) : (
              <OverlayTrigger
              ...
              >
                <i
                  className={`fa-regular fa-calendar-check ${styles.AttendUnauth}`}
                />
              </OverlayTrigger>
            )}
            {attendings_count}
